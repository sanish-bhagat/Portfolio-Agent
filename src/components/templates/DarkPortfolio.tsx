...
const DarkPortfolio = ({ portfolioData, visibleSections = [] }: Props) => {
  ...
  const isVisible = (id: string) => visibleSections.length === 0 || visibleSections.includes(id);
      ...
      {isVisible('hero') && (
        <section className="min-h-[90vh] flex flex-col items-center justify-center px-6 text-center relative">
          ...
          <div className="relative z-10 max-w-3xl mx-auto">
            ...
            <p className="text-lg md:text-xl text-dark-theme-muted-fg max-w-2xl mx-auto mb-10 leading-relaxed">{hero.tagline}</p>
      ...
      {isVisible('contact') && (
        <section id="contact" className="py-20 md:py-28 px-6">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="flex flex-wrap justify-center gap-6 mb-8 text-dark-theme-muted-fg font-mono-jet">
              {contact_section.phone && (
                <a href={`tel:${contact_section.phone}`} className="flex items-center gap-2 hover:text-dark-theme-accent transition-colors">
                  <Phone className="w-4 h-4 text-dark-theme-accent" />
                  <span>{contact_section.phone}</span>
                </a>
              )}
              {contact_section.location && (
                <div className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-dark-theme-accent" />
                  <span>{contact_section.location}</span>
                </div>
              )}
            </div>
