    ...
    <div className="min-h-screen bg-dark-theme-bg text-dark-theme-fg font-space">
      ...
      <section className="min-h-[90vh] flex flex-col items-center justify-center px-6 text-center relative">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-6 leading-tight">
            Hi, I'm{" "}
            <span className="dark-gradient-text">{hero.name}</span>
          </h1>
          <p className="text-lg md:text-xl text-dark-theme-muted-fg max-w-2xl mx-auto mb-4 leading-relaxed">
            {hero.tagline}
          </p>
          {hero.location && (
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="#projects" className="bg-dark-theme-accent text-dark-theme-accent-fg px-8 py-3.5 rounded-lg font-semibold text-base hover:brightness-110 transition-all dark-glow">
              View My Work
            </a>
            <a href="#contact" className="px-8 py-3.5 rounded-lg font-semibold text-base border border-dark-theme-border text-dark-theme-fg hover:border-dark-theme-accent hover:text-dark-theme-accent transition-all">
              Contact Me
            </a>
          </div>
        </div>
      ...
      <section className="py-20 md:py-28 px-6 bg-dark-theme-card">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-6">
            <p className="text-xs uppercase tracking-wider text-dark-theme-muted-fg mb-3 font-mono-jet">Primary Stack</p>
            <div className="flex flex-wrap gap-2">
              {skills_section.primary_skills.map((s) => (
                <span key={s} className="px-3 py-1.5 rounded-md bg-dark-theme-tag-bg text-dark-theme-tag-fg text-sm font-medium border border-dark-theme-accent/20">
                  {s}
                </span>
              ))}
            </div>
          ...
          <div>
            <p className="text-xs uppercase tracking-wider text-dark-theme-muted-fg mb-3 font-mono-jet">Also Using</p>
            <div className="flex flex-wrap gap-2">
              {skills_section.secondary_skills.map((s) => (
                <span key={s} className="px-3 py-1.5 rounded-md bg-dark-theme-tag-secondary-bg text-dark-theme-tag-secondary-fg text-sm">
                  {s}
                </span>
              ))}
            </div>
                ...
                {exp.tech_stack.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {exp.tech_stack.map((t) => (
                      <span key={t} className="text-xs px-2 py-0.5 rounded bg-dark-theme-tag-bg text-dark-theme-tag-fg font-mono-jet">{t}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {projects_section.map((proj, i) => (
              <div key={i} className="bg-dark-theme-bg rounded-xl p-6 border border-dark-theme-border hover:border-dark-theme-accent/40 transition-all group">
                <h3 className="text-xl font-semibold text-dark-theme-fg mb-2 group-hover:text-dark-theme-accent transition-colors">{proj.title}</h3>
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-4 mt-2">
            {contact_section.linkedin && (
              <a href={contact_section.linkedin} target="_blank" rel="noreferrer" className="p-3 rounded-lg bg-dark-theme-card text-dark-theme-muted-fg hover:text-dark-theme-accent border border-dark-theme-border hover:border-dark-theme-accent/40 transition-all">
                <Linkedin className="w-5 h-5" />
              </a>
            )}
            {contact_section.github && (
              <a href={contact_section.github} target="_blank" rel="noreferrer" className="p-3 rounded-lg bg-dark-theme-card text-dark-theme-muted-fg hover:text-dark-theme-accent border border-dark-theme-border hover:border-dark-theme-accent/40 transition-all">
                <Github className="w-5 h-5" />
              </a>
            )}
            {contact_section.portfolio && (
              <a href={contact_section.portfolio} target="_blank" rel="noreferrer" className="p-3 rounded-lg bg-dark-theme-card text-dark-theme-muted-fg hover:text-dark-theme-accent border border-dark-theme-border hover:border-dark-theme-accent/40 transition-all">
                <Globe className="w-5 h-5" />
              </a>
            )}
            {contact_section.phone && (
              <a href={`tel:${contact_section.phone}`} className="p-3 rounded-lg bg-dark-theme-card text-dark-theme-muted-fg hover:text-dark-theme-accent border border-dark-theme-border hover:border-dark-theme-accent/40 transition-all">
      ...
      <footer className="py-8 text-center text-xs text-dark-theme-muted-fg border-t border-dark-theme-border font-mono-jet">
        © {new Date().getFullYear()} {hero.name} — built with ♥
      </footer>
    </div>
  );
};
