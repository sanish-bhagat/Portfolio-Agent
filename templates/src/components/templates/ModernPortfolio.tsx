...
import { MapPin, Mail, ExternalLink, Github, Linkedin, ArrowDown, Phone, Globe } from "lucide-react";
...
const SectionHeading = ({ children }: { children: React.ReactNode }) => (
  <h2 className="text-3xl md:text-4xl font-space font-bold text-modern-fg mb-2">
    {children}
  </h2>
);
...
const Tag = ({ children, variant = "primary" }: { children: React.ReactNode; variant?: "primary" | "secondary" }) => (
  <span
    className={`inline-block px-3 py-1.5 rounded-full text-sm font-medium transition-transform hover:scale-105 ${
      variant === "primary"
        ? "bg-modern-tag-bg text-modern-tag-fg"
        : "bg-modern-tag-secondary-bg text-modern-tag-secondary-fg"
    }`}
  >
    {children}
  </span>
    ...
    <div className="min-h-screen bg-modern-bg font-inter">
      ...
      <section className="min-h-[90vh] flex flex-col items-center justify-center px-6 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          ...
          <h1 className="text-5xl md:text-7xl font-space font-bold text-modern-fg mb-6 leading-tight">
            Hi, I'm{" "}
            <span className="modern-gradient-text">{hero.name}</span>
          </h1>
          <p className="text-lg md:text-xl text-modern-muted-fg max-w-2xl mx-auto mb-10 leading-relaxed">
            {hero.tagline}
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="#projects" className="modern-gradient-bg text-modern-accent-fg px-8 py-3.5 rounded-full font-semibold text-base shadow-lg shadow-modern-accent/25 hover:shadow-xl hover:shadow-modern-accent/30 transition-all hover:-translate-y-0.5">
              View My Work
            </a>
            <a href="#contact" className="px-8 py-3.5 rounded-full font-semibold text-base border-2 border-modern-muted text-modern-fg hover:border-modern-accent hover:text-modern-accent transition-all">
              Contact Me
            </a>
          </div>
        </div>
      ...
      <section className="py-20 md:py-28 px-6 bg-modern-card">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="mb-6">
            <p className="text-sm font-semibold text-modern-muted-fg uppercase tracking-wider mb-3">Core</p>
            <div className="flex flex-wrap gap-2">
              {skills_section.primary_skills.map((s) => <Tag key={s}>{s}</Tag>)}
            </div>
          </div>
          <div>
            <p className="text-sm font-semibold text-modern-muted-fg uppercase tracking-wider mb-3">Also Proficient In</p>
            <div className="flex flex-wrap gap-2">
              {skills_section.secondary_skills.map((s) => <Tag key={s} variant="secondary">{s}</Tag>)}
            </div>
          </div>
        </div>
      </section>
                ...
                {exp.tech_stack.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {exp.tech_stack.map((t) => (
                      <span key={t} className="text-xs px-2 py-0.5 rounded bg-modern-tag-bg text-modern-tag-fg">{t}</span>
                    ))}
                  </div>
                )}
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {projects_section.map((proj, i) => (
              <div key={i} className="bg-modern-bg rounded-2xl p-6 modern-card-hover border border-modern-muted/50">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {proj.tech_stack.map((t) => <Tag key={t}>{t}</Tag>)}
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-4 mt-2">
            {contact_section.linkedin && (
              <a href={contact_section.linkedin} target="_blank" rel="noreferrer" className="p-3 rounded-full bg-modern-card text-modern-muted-fg hover:text-modern-accent transition-colors shadow-sm">
                <Linkedin className="w-5 h-5" />
              </a>
            )}
            {contact_section.github && (
              <a href={contact_section.github} target="_blank" rel="noreferrer" className="p-3 rounded-full bg-modern-card text-modern-muted-fg hover:text-modern-accent transition-colors shadow-sm">
                <Github className="w-5 h-5" />
              </a>
            )}
            {contact_section.portfolio && (
              <a href={contact_section.portfolio} target="_blank" rel="noreferrer" className="p-3 rounded-full bg-modern-card text-modern-muted-fg hover:text-modern-accent transition-colors shadow-sm">
