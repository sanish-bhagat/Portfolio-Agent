...
import { MapPin, Mail, ArrowDown, Github, Linkedin, Phone, Globe, ArrowUpRight } from "lucide-react";
    ...
    <div className="min-h-screen bg-minimal-bg font-inter text-minimal-fg">
      ...
      <section className="min-h-[85vh] flex flex-col justify-center px-6 md:px-12">
        <div className="max-w-[720px] mx-auto w-full">
          ...
          <p className="text-lg md:text-xl text-minimal-muted-fg leading-relaxed mb-6 max-w-lg">
            {hero.tagline}
          </p>
          {hero.location && (
      ...
      {/* Divider */}
      <div className="max-w-[720px] mx-auto px-6 md:px-12"><hr className="border-minimal-border" /></div>
          ...
          <div className="mb-6">
            <div className="flex flex-wrap gap-2">
              {skills_section.primary_skills.map((s) => (
                <span key={s} className="px-3 py-1.5 border border-minimal-fg text-sm font-medium">{s}</span>
              ))}
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {skills_section.secondary_skills.map((s) => (
              <span key={s} className="px-3 py-1.5 border border-minimal-border text-sm text-minimal-muted-fg">{s}</span>
            ))}
          </div>
        </div>
      </section>
                ...
                {exp.tech_stack.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {exp.tech_stack.map((t) => (
                      <span key={t} className="text-xs text-minimal-muted-fg bg-minimal-muted px-2 py-0.5">{t}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className="max-w-[720px] mx-auto">
          ...
          <div className="flex gap-6 text-minimal-muted-fg">
            {contact_section.linkedin && (
              <a href={contact_section.linkedin} target="_blank" rel="noreferrer" className="hover:text-minimal-fg transition-colors"><Linkedin className="w-5 h-5" /></a>
            )}
            {contact_section.github && (
              <a href={contact_section.github} target="_blank" rel="noreferrer" className="hover:text-minimal-fg transition-colors"><Github className="w-5 h-5" /></a>
            )}
            {contact_section.portfolio && (
              <a href={contact_section.portfolio} target="_blank" rel="noreferrer" className="hover:text-minimal-fg transition-colors"><Globe className="w-5 h-5" /></a>
            )}
            {contact_section.phone && (
              <a href={`tel:${contact_section.phone}`} className="hover:text-minimal-fg transition-colors"><Phone className="w-5 h-5" /></a>
            )}
          </div>
        </div>
      </section>
      ...
      <footer className="py-8 px-6 md:px-12 text-xs text-minimal-muted-fg border-t border-minimal-border">
        <div className="max-w-[720px] mx-auto">© {new Date().getFullYear()} {hero.name}</div>
      </footer>
    </div>
  );
};
